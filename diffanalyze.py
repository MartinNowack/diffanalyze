...
class OutputManager:
    ...
    def print_all(only_fn):
        lines = OutputManager.output.getvalue().strip().split('\n') if not only_fn else list(
...
class FileDifferences:
    ...
    def get_fn_names(self, prev):
        ...
        proc = subprocess.Popen(['ctags', '-x', '--c-kinds=fp', '--fields=+ne', '--output-format=json', target],
...
class RepoManager:
    ...
    def order_results(self, other=False):
        target = self.other_changed.items() if other else self.fn_updated_per_commit.items()
...
def main(main_args):
    ...
    parser.add_argument('-rh', '--range', metavar='INIT_HASH',
                        help='look at patches between INIT_HASH and HASH (inclusive)')
